<template>
  <div class="content" :style="{ height: isshow ? '800px' : '400px' }">
    ...
    <div class="bottom" v-if="isshow">
      <div
        class="bottomLeft"
        @mouseover="mouseHover('D', echartsBottoms, businessId.data.left)"
      >
        <div class="con" :id="echartsBottoms"></div>
        <div class="ListTable" :id="echartsBottoms + 1"></div>
        <information
          @isData="isData"
          class="information"
          ref="inform"
...
export default {
  ...
  methods: {
    ...
    EchartsBottom() {
      let echartsBottom = this.$echarts.init(
        document.getElementById(this.echartsBottoms)
      );
  ...
  mounted() {
    ...
    //         this_.resizeData.forEach(element => {
    //             element.resize()
    //         });
    //     }
    // }, 5000);
