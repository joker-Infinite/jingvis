<template>
    <div class="businesstop">
        ...
        <el-collapse id="box1" v-model="revenue" @change="handleChange">
            <el-collapse-item :name="(i + 1).toString()" v-for="(item, i) of businessId" :key="i">
                <template slot="title">
        ...
        <el-collapse id="box2" v-model="profit" @change="handleChange">
            <el-collapse-item :name="(i + 1).toString()" v-for="(item, i) of profitbusinessId" :key="i">
                <template slot="title">
                    <span style="font-weight:bold;font-size:14px;color:#fff;margin-left:20px">{{ item.name }}</span>
                </template>
                <businessTopEcharts ref="busin" :businessId="item"></businessTopEcharts>
        ...
        <el-collapse id="box3" v-model="cost" @change="handleChange">
            <el-collapse-item :name="(i + 1).toString()" v-for="(item, i) of costbusinessId" :key="i">
                <template slot="title">
                    <span style="font-weight:bold;font-size:14px;color:#fff;margin-left:20px">2020年成本</span>
                </template>
                <businessTopEcharts ref="busin" :businessId="item" :isshow="true"></businessTopEcharts>
            </el-collapse-item>
        </el-collapse>
        <!-- 成本结束 -->
    </div>
...
export default {
    ...
    methods: {
        ...
        isechartsShow() {
          // console.log(this.$refs.busin.isExcle,555)
          // this.$refs.busin.forEach(element => {
          //   element.isechartsShow()
          // });
            // this.$refs.busin.isechartsShow();
        }
    },
    mounted() {
