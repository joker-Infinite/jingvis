<template>
    <div class="businessBottom">
        ...
        <el-collapse v-model="quantitativeStructure" @change="handleChange" >
            <el-collapse-item id="box4" name="1" v-if="quantityShow">
                <template slot="title">
                    <span style="font-weight:bold;font-size:14px;color:#fff;margin-left:20px">服务区数量</span>
                </template>
                <div class="businessEchartsFWQhover">
                  <div id="businessEchartsFWQ"></div>
                  <information @isData="isData" class="information" ref="inform"></information>
                </div>
            </el-collapse-item>
            <div id="box5"></div>
            <el-collapse-item :name="(2 + index).toString()" v-for="(item, index) of quantitativeStructurebusinessId" :key="index">
                <template slot="title">
                    <span style="font-weight:bold;font-size:14px;color:#fff;margin-left:20px">{{ item.name }}</span>
                </template>
                <business-bottom-echarts v-for="(id, i) of item.id" :key="i" ref="businessButton" :businessId="id"></business-bottom-echarts>
            </el-collapse-item>
        </el-collapse>
        <!--数量结构结束  -->
        <!-- 回款率开始 -->
        <el-collapse id="box6" v-model="rateofreturn" @change="handleChange">
            <el-collapse-item :name="(index + 1).toString()" v-for="(item, index) of rateofreturnbusinessId" :key="index">
                <template slot="title">
                    <span style="font-weight:bold;font-size:14px;color:#fff;margin-left:20px">{{ item.name }}</span>
                </template>
                <business-bottom-echarts1 ref="businessButton1" :businessId="item"></business-bottom-echarts1>
            </el-collapse-item>
        </el-collapse>
        <!-- 回款率结束 -->
    </div>
            ...
            var option = {
                tooltip: {
                    ...
                    formatter: function (val) {
                        return val[0].name + ':'+"<br>" +  val[0].marker +
                        val[0].seriesName + ":" +val[0].value + "元" + '<br>' +val[1].marker +
                        val[1].seriesName + ":"+  val[1].value + "元" + "<br>" + val[2].marker +
                        val[2].seriesName + ":"+  val[1].value + "元" + "<br>" 
                    },
                    textStyle: {
                        color: "#fff",
                        fontStyle: "normal",
...
.businessEchartsFWQhover{
  width: 100%;
  height: 100%;
  position: relative;
}
.businessEchartsFWQhover:hover > .information{
  display: block;
}
</style>
